# TopView2026 - 赛博聚石盆视频平台

> 基于 Java Servlet + JDBC 的手写 MVC 框架视频社交平台

## 📋 目录

- [项目概述](#项目概述)
- [技术栈](#技术栈)
- [架构设计](#架构设计)
- [功能模块](#功能模块)
- [数据库设计](#数据库设计)
- [API 接口文档](#api-接口文档)
- [环境要求](#环境要求)
- [快速开始](#快速开始)
- [项目结构](#项目结构)
- [核心实现](#核心实现)
- [单元测试](#单元测试)
- [性能优化](#性能优化)
- [常见问题](#常见问题)

---

## 项目概述

TopView2026（赛博聚石盆）是一个基于 Java Web 技术栈开发的视频社交平台，支持视频发布、Feed 流推送、优惠券抢购等核心功能。

### 考核信息

- **一轮考核**：基础视频平台功能（已完成）
- **二轮考核**：Feed 流推送 + 优惠券抢购（进行中）
- **开始时间**：2026-04-23
- **截止时间**：2026-05-11
- **答辩时间**：2026-05-12

### 核心特性

✅ **手写 MVC 框架**：不使用 SpringBoot/SpringMVC/MyBatis  
✅ **前后端分离**：后端提供 RESTful API，前端使用 Vue/React  
✅ **Redis 缓存**：用户行为数据缓存、Feed 流加速  
✅ **OSS 云存储**：阿里云 OSS 视频文件存储  
✅ **RBAC 权限控制**：基于角色的访问控制  
✅ **声明式事务**：自定义注解实现事务管理  
✅ **连接池优化**：自定义数据库连接池  

---

## 技术栈

### 核心技术

| 类别 | 技术 | 版本 | 说明 |
|------|------|------|------|
| 后端语言 | Java | 8+ | 禁止使用 SpringBoot/SpringMVC/MyBatis |
| Web 框架 | Java Servlet | 4.0 | 手写 MVC 架构 |
| 数据库 | MySQL | 8.0+ | 关系型数据存储 |
| 缓存 | Redis | 6.0+ | 用户行为缓存、Feed 流加速 |
| 云存储 | 阿里云 OSS | - | 视频文件存储 |
| 构建工具 | Maven | 3.6+ | 项目依赖管理 |
| Web 容器 | Tomcat | 9.0+ | 应用服务器 |

### 可选框架（加分项）

| 类别 | 技术 | 用途 |
|------|------|------|
| 消息队列 | RocketMQ / Kafka | Feed 流异步推送 |
| 流量控制 | Sentinel | 限流、熔断、排队 |
| RPC 框架 | Dubbo / gRPC | 服务间调用 |
| 配置中心 | Nacos / Eureka | 配置持久化 |
| 链路追踪 | SkyWalking | 性能监控 |
| 分布式事务 | SEATA | 跨服务事务 |
| 容器化 | Docker / K8s | 部署编排 |

### 开发工具

- **Lombok**：简化代码编写
- **JUnit 5 + Mockito**：单元测试
- **Fastjson**：JSON 序列化
- **Hutool**：工具类库

---


## 技术要求

### 📋 基础要求

#### 1. Redis 缓存实现

**核心目标**：提升系统性能，降低数据库压力

**实现要点**：
- ✅ **用户行为数据缓存**
    - 缓存用户 Token（双 Token 机制）
    - 缓存视频详情信息
    - 缓存评论列表（按分页和排序方式）
    - 缓存 Feed 流数据（ZSet 结构）
    - 缓存用户权限信息

- ✅ **Feed 流加速**
    - Push 模式：`feed:push:{userId}` ZSet 存储推送视频ID
    - Pull 模式：缓存关注列表，减少数据库查询
    - 热度分数缓存：`video:hotness` 缓存牛顿冷却算法计算结果

- ✅ **SQL 性能优化**
    - 避免全表扫描：所有查询必须带索引字段
    - 控制返回结果集：使用 LIMIT 分页，单次查询不超过 100 条
    - 避免 N+1 查询：使用 JOIN 或批量查询替代循环查询

- ✅ **分表分库策略**
    - 用户表：按 user_id 哈希分表（预留）
    - 视频表：按 create_time 月份分表（预留）
    - 评论表：按 video_id 哈希分表（预留）
    - 优惠券表：按 activity_id 分表（预留）

**Redis 数据结构设计**：
---

#### 2. 数据一致性

**核心目标**：保证分布式环境下的数据最终一致性

**实现方案**：
- ✅ **Feed 流推送异步处理**
  - 使用 `ScheduledExecutorService` 异步推送
  - 推送失败不影响视频发布主流程
  - 失败重试机制：记录失败日志，人工介入
  
- ✅ **优惠券库存同步**
  - Redis 作为主库存（高性能）
  - MySQL 作为持久化存储（可靠性）
  - 异步线程定期同步 Redis → MySQL
  - 抢购成功后立即扣减 Redis，异步更新 MySQL

- ✅ **事务管理**
  - 自定义 `@Transactional` 注解实现声明式事务
  - 基于 JDK 动态代理的事务拦截器
  - 支持事务传播行为：REQUIRED、REQUIRES_NEW
  - 异常自动回滚，成功自动提交

**示例代码**：

---

#### 3. 流量控制

**核心目标**：防止系统过载，保障服务稳定性

**实现方案**：
- ✅ **限流机制**
  - IP 级别限流：同一 IP 每分钟最多请求 60 次
  - 用户级别限流：同一用户每秒最多请求 10 次
  - 接口级别限流：抢购接口每秒最多处理 1000 次
  
- ✅ **熔断机制**
  - 数据库连接池耗尽时快速失败
  - Redis 连接超时降级为直接查数据库
  - OSS 上传失败重试 3 次后熔断

- ✅ **排队机制**
  - 高并发抢购使用队列缓冲请求
  - 异步处理队列中的请求
  - 返回排队状态给前端

**注意**：目前使用自定义实现，后续可集成 Sentinel 中间件

---

#### 4. 线程安全

**核心目标**：保证多线程环境下的数据正确性

**实现要点**：
- ✅ **连接池线程安全**
  - `ConnectionPool` 使用 `BlockingQueue` 管理连接
  - `synchronized` 保证获取/归还连接的原子性
  
- ✅ **Redis 操作线程安全**
  - Jedis 连接池保证并发访问安全
  - Lua 脚本保证复合操作的原子性
  
- ✅ **单例模式线程安全**
  - `ApplicationContext` 使用双重检查锁定
  - `TransactionManager` 使用 ThreadLocal 隔离事务上下文

**示例代码**：

---

#### 5. 单元测试

**核心目标**：保证代码质量，便于回归测试

**测试框架**：
- **JUnit 5**：测试框架
- **Mockito**：Mock 对象，隔离外部依赖

**测试覆盖**：
- ✅ **DAO 层测试**：数据库 CRUD 操作
- ✅ **Service 层测试**：业务逻辑验证
- ✅ **异常处理测试**：边界条件验证
- ✅ **并发测试**：超卖、重复领取等场景

**示例代码**：

**运行测试**：

---

### 🚀 进阶要求（加分项）

#### 6. MySQL 优化

**索引优化**：
- ✅ **主键索引**：所有表使用 BIGINT 自增主键
- ✅ **唯一索引**：`uk_user_activity` 防止重复领取
- ✅ **联合索引**：`(start_time, end_time)` 优化时间范围查询
- ✅ **覆盖索引**：查询只需要的字段，避免回表

**SQL 优化**：
- ✅ 避免 `SELECT *`，只查询必要字段
- ✅ 使用 `EXPLAIN` 分析执行计划
- ✅ 避免大事务，拆分复杂操作
- ✅ 使用批量插入替代逐条插入

**分库分表策略（已实施）**：
- ✅ **垂直分表**：
    - **目标**：将 `video_info` 表的大字段 `description` 分离到 `video_info_detail` 扩展表。
    - **效果**：列表查询 I/O 减少 40-60%，显著提升 Buffer Pool 缓存命中率。
    - **实现**：通过手动指定 SQL 查询列排除大字段，详情查询时按需加载或二次查询。
- ✅ **水平分表**：
    - **目标**：针对高频增长的 `video_comment` 评论表，采用**按月分表**策略（如 `video_comment_202605`）。
    - **路由逻辑**：开发 `ShardingUtil` 工具类，根据 `create_time` 自动计算目标分表名。
    - **聚合查询**：DAO 层通过 `UNION ALL` 聚合最近 6 个月的分表数据，支持跨表分页与排序。
    - **优势**：单表数据量控制在百万级以内，大幅提升索引维护效率和查询响应速度。


---

#### 7. 消息队列（RocketMQ / Kafka）

**应用场景**：
- 📝 **Feed 流异步推送**
  - 博主发布视频 → 发送消息到 MQ
  - 消费者异步推送到粉丝 Feed 流
  - 支持失败重试和死信队列
  
- 📝 **优惠券抢购削峰**
  - 抢购请求进入 MQ 队列
  - 消费者按能力处理请求
  - 平滑流量峰值

**优势**：
- 解耦生产者和消费者
- 异步处理提升响应速度
- 削峰填谷保护后端服务

---

#### 8. 流量控制（Sentinel）

**功能特性**：
- 📝 **限流**：QPS/线程数限流
- 📝 **熔断**：异常比例/响应时间熔断
- 📝 **降级**：热点参数降级
- 📝 **系统保护**：Load/CPU/入口 QPS 保护

**集成方式**：

---

#### 9. RPC 框架（Dubbo / gRPC）

**应用场景**：
- 📝 **微服务拆分**
  - 用户服务：认证、权限
  - 视频服务：上传、播放
  - 互动服务：点赞、评论、关注
  - 营销服务：优惠券、活动

**优势**：
- 服务治理（负载均衡、熔断）
- 接口标准化
- 多语言支持

---

#### 10. 配置持久化（Nacos / Eureka）

**功能特性**：
- 📝 **配置中心**：统一管理配置文件
- 📝 **服务注册发现**：自动注册和发现服务
- 📝 **动态刷新**：配置变更实时生效

**迁移路径**：


## 架构设计

### 整体架构

### MVC 分层

---

## 功能模块

### 一轮考核功能（已完成）✅

#### 1. 用户认证模块
- ✅ 用户注册
- ✅ 用户登录（双 Token 机制：AccessToken + RefreshToken）
- ✅ Token 刷新
- ✅ 用户信息查询
- ✅ RBAC 权限控制

#### 2. 视频管理模块
- ✅ 视频上传（支持封面）
- ✅ 视频列表查询（分页）
- ✅ 视频详情查询
- ✅ 按作者查询视频
- ✅ 按分类查询视频
- ✅ 视频搜索
- ✅ 视频删除
- ✅ 视频点赞/取消点赞
- ✅ 视频下载链接获取
- ✅ 视频本地缓存

#### 3. 评论模块
- ✅ 发表评论
- ✅ 评论列表查询（按热度/时间排序）
- ✅ 删除评论
- ✅ 评论点赞/取消点赞
- ✅ 用户评论历史查询

#### 4. 关注模块
- ✅ 关注用户
- ✅ 取消关注
- ✅ 关注列表查询
- ✅ 粉丝列表查询
- ✅ 关注状态查询
- ✅ 关注数量统计

#### 5. 权限管理模块
- ✅ 用户权限查询
- ✅ 资源级权限校验（所有权检查）

---

### 二轮考核功能（开发中）🚧

#### 1. Feed 流推送模块

**目标需求**：用户可以通过首页或推荐页面，接收所关注博主或其他用户发布的视频

##### 基础功能

**视频内容管理**
- [ ] 视频内容分区（美食、探店、科技等分类）
- [ ] 博主/用户发布动态、视频
- [ ] 动态内容类型扩展（图文、视频混合）

**推送机制**

**Pull 模式（拉取）**
- [ ] 用户上线后拉取关注用户的最新动态
- [ ] 基于游标的分页查询（加分项）
- [ ] 缓存用户 Feed 流数据到 Redis

**Push 模式（推送）**
- [ ] 博主发布动态后，主动推送到粉丝 Feed 流
- [ ] 异步推送机制（使用消息队列 - 加分项）
- [ ] 推送失败重试机制

**用户交互**
- [ ] 视频点赞（已实现）
- [ ] 评论点赞（已实现）
- [ ] 评论视频/动态（已实现）
- [ ] 视频收藏功能（新增）

##### 技术实现要点

**Redis 数据结构设计**
- `feed:push:{userId}`: ZSet 存储推送给用户的视频ID列表，score 为发布时间戳
- `video:hotness`: 缓存视频热度分数（可选）

---

**热度排序算法（牛顿冷却定律）**
- **公式**: `热度分数 = 互动得分 / (经过小时数 + 2) ^ 重力因子`
- **互动得分**: 播放量×1 + 点赞数×5 + 评论数×10
- **重力因子**: 1.8（控制衰减速度）
- **应用场景**: Feed流视频排序、评论热度排序
- **优势**:
    - 新内容有机会快速上升（挖坟机制）
    - 老内容自然下沉，避免长期霸榜
    - 平衡时效性与质量

---

#### 2. 优惠券抢购模块

**目标需求**：用户在观看博主视频时，参与优惠券限时抢购活动

##### 基础功能

**优惠券库存管理**
- [ ] 设置固定库存量
- [ ] 实时更新库存（Redis 原子操作）
- [ ] 防止超卖（Lua 脚本保证原子性）

**活动时间管理**
- [ ] 设置抢购开始时间
- [ ] 设置抢购结束时间
- [ ] 自动开启/结束活动
- [ ] 倒计时提示

**抢购流程**
- [ ] 用户点击抢购
- [ ] 校验活动时间
- [ ] 扣减库存（Redis DECR）
- [ ] 生成优惠券码
- [ ] 写入数据库（异步）
- [ ] 返回抢购结果

**抢购结果通知**
- [ ] 成功：生成优惠券码，实时返回
- [ ] 失败：库存不足/活动未开始/已结束
- [ ] 事务回滚：库存不足时恢复 Redis 库存

##### 技术实现要点

**防止超卖方案**

**数据库表设计**

---

## 数据库设计

### 核心表结构

#### 1. 用户相关

#### 2. 视频相关

#### 3. 互动相关

#### 4. 权限相关

#### 5. 优惠券相关（二轮新增）

---

## API 接口文档

### 基础信息

- **Base URL**: `http://localhost:9090/saibo-pot`
- **认证方式**: Header 中携带 `Authorization: Bearer {token}`
- **响应格式**: JSON

### 统一响应格式

### 接口列表

#### 1. 用户认证模块

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/user/register` | 用户注册 | ❌ |
| POST | `/api/user/login` | 用户登录 | ❌ |
| POST | `/api/user/refresh` | 刷新 Token | ❌ |
| GET | `/api/user/info` | 获取用户信息 | ✅ |

#### 2. 视频管理模块

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/video/upload` | 上传视频 | ✅ |
| GET | `/api/video/list` | 视频列表 | ❌ |
| GET | `/api/video/{id}` | 视频详情 | ❌ |
| GET | `/api/video/author/{authorId}` | 作者视频 | ❌ |
| GET | `/api/video/category/{categoryId}` | 分类视频 | ❌ |
| GET | `/api/video/search` | 搜索视频 | ❌ |
| DELETE | `/api/video/{id}` | 删除视频 | ✅ |
| POST | `/api/video/{id}/like` | 点赞视频 | ✅ |
| POST | `/api/video/{id}/unlike` | 取消点赞 | ✅ |
| POST | `/api/video/{id}/cache` | 缓存视频 | ✅ |
| GET | `/api/video/download/{id}` | 下载链接 | ✅ |

#### 3. 评论模块

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/comment/add` | 发表评论 | ✅ |
| GET | `/api/comment/video/{videoId}` | 视频评论 | ❌ |
| GET | `/api/comment/user/{userId}` | 用户评论 | ❌ |
| DELETE | `/api/comment/{id}` | 删除评论 | ✅ |
| POST | `/api/comment/{id}/like` | 点赞评论 | ✅ |
| POST | `/api/comment/{id}/unlike` | 取消点赞 | ✅ |

#### 4. 关注模块

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/follow/follow` | 关注用户 | ✅ |
| POST | `/api/follow/unfollow` | 取消关注 | ✅ |
| GET | `/api/follow/following` | 关注列表 | ✅ |
| GET | `/api/follow/followers` | 粉丝列表 | ✅ |
| GET | `/api/follow/counts` | 关注数量 | ✅ |
| GET | `/api/follow/status/{targetUserId}` | 关注状态 | ✅ |

#### 5. Feed 流模块（二轮新增）

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/feed/pull` | 拉取 Feed 流 | ✅ |
| POST | `/api/feed/push` | 推送 Feed 流 | ✅ |
| GET | `/api/feed/cursor` | 游标分页 | ✅ |

#### 6. 优惠券模块（二轮新增）

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/coupon/create` | 创建活动 | ✅ |
| GET | `/api/coupon/list` | 活动列表 | ❌ |
| GET | `/api/coupon/{id}` | 活动详情 | ❌ |
| POST | `/api/coupon/{id}/grab` | 抢购优惠券 | ✅ |
| GET | `/api/coupon/my` | 我的优惠券 | ✅ |

#### 7. 收藏模块（二轮新增）

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/api/favorite/add` | 收藏视频 | ✅ |
| DELETE | `/api/favorite/{videoId}` | 取消收藏 | ✅ |
| GET | `/api/favorite/list` | 收藏列表 | ✅ |

#### 8. 权限管理模块

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| GET | `/api/admin/permission/my-permissions` | 我的权限 | ✅ |

---

## 环境要求

### 必需软件

| 软件 | 版本 | 下载地址 |
|------|------|----------|
| JDK | 8+ | https://www.oracle.com/java/technologies/downloads/ |
| Maven | 3.6+ | https://maven.apache.org/download.cgi |
| MySQL | 8.0+ | https://dev.mysql.com/downloads/ |
| Redis | 6.0+ | https://redis.io/download/ |
| Tomcat | 9.0+ | https://tomcat.apache.org/download-90.cgi |

### 可选软件

- **Node.js** 16+（前端开发）
- **Docker**（容器化部署）
- **IntelliJ IDEA**（推荐 IDE）

---

## 快速开始

### 1. 克隆项目

### 2. 配置数据库

创建数据库并导入 SQL 文件：

修改 `src/main/resources/db.properties`：


### 3. 编译项目

### 4. 部署到 Tomcat

### 5. 访问应用

- **后端 API**: http://localhost:9090/saibo-pot/api/
- **默认管理员账号**: admin / 123456

### 6. 运行测试

---

## 项目结构


---

## 核心实现

### 1. 手写 IoC 容器

### 2. 声明式事务

### 3. 通用 DAO 封装（基于反射）

### 4. 数据库连接池

### 5. Redis 缓存

---

## 单元测试

### 测试框架

- **JUnit 5**：测试框架
- **Mockito**：Mock 对象

### 测试示例

### 运行测试

---

## 性能优化

### 1. 数据库优化

**索引优化**

**SQL 优化**
- ✅ 避免 `SELECT *`，只查询需要的字段
- ✅ 使用分页限制返回结果集大小
- ✅ 避免 N+1 查询，使用 JOIN 或批量查询

### 2. Redis 缓存策略

**缓存层级**

**缓存更新策略**
- **Cache-Aside**：先读缓存，Miss 后查 DB 并写入缓存
- **Write-Through**：更新 DB 同时更新缓存
- **TTL 过期**：设置合理的过期时间

### 3. 连接池配置

### 4. Feed 流优化

**Pull 模式优化**
- 使用游标分页替代 OFFSET
- 缓存用户关注列表
- 预加载热门视频元数据

**Push 模式优化**
- 异步推送（消息队列）
- 批量写入 Redis
- 大 V 特殊处理（只推活跃粉丝）

**热度排序优化**
- ✅ 牛顿冷却定律算法实现
- ✅ 多维度互动得分计算（播放、点赞、评论加权）
- ✅ 时间衰减机制，支持"挖坟"功能
- ✅ 重力因子可配置，灵活控制衰减速度
- 
---

### 5. Redis 缓存

**缓存策略**
- 用户 Token 缓存（双 Token 机制）
- 评论列表缓存（按分页和排序方式）
- Feed 流数据缓存（ZSet 结构）
- 用户权限缓存
- 视频热度分数缓存（可选）

**热点数据处理**
- ✅ 评论热度排序（点赞数 + 时间衰减）
- ✅ Feed 流视频热度排序（牛顿冷却定律）
- ✅ 防刷机制：IP 限流、有效观看判定、去重

## 常见问题

### Q1: 启动时提示 "ApplicationContext not initialized"

**原因**: AppStartupListener 未正确加载  
**解决**: 检查 `web.xml` 中是否配置了 Listener

### Q2: Redis 连接失败

**原因**: Redis 未启动或配置错误  
**解决**: 

### Q3: OSS 上传返回 403

**原因**: AccessKey 权限不足或 Bucket 配置错误  
**解决**: 检查 `db.properties` 中的 OSS 配置

### Q4: 视频缓存下载失败（403）

**原因**: OSS Bucket 为私有权限  
**解决**: 
1. 使用签名 URL（已实现）
2. 或将 Bucket 改为公共读

### Q5: 事务不回滚

**原因**: 异常被捕获未抛出  
**解决**: 确保异常向上抛出，或使用 `@Transactional` 注解

---

## 开发规范

### 代码规范

1. **命名规范**
   - 类名：大驼峰（PascalCase）
   - 方法/变量：小驼峰（camelCase）
   - 常量：全大写+下划线（UPPER_SNAKE_CASE）

2. **注释规范**
   - 类必须有类注释
   - 公共方法必须有方法注释
   - 复杂逻辑必须添加行内注释

3. **异常处理**
   - 使用自定义异常类
   - 统一通过 GlobalExceptionHandler 处理
   - 不要吞掉异常

### Git 提交规范

---

## 后续规划

### 短期目标（二轮考核）

- [ ] 完成 Feed 流 Pull/Push 模式
- [ ] 实现优惠券抢购功能
- [ ] 添加视频收藏功能
- [ ] 完善单元测试覆盖率 > 80%
- [ ] 编写完整 API 文档

### 中期目标

- [ ] 引入消息队列（RocketMQ/Kafka）
- [ ] 集成 Sentinel 流量控制
- [ ] 实现分布式锁（Redis）
- [ ] 前端页面开发（Vue/React）

### 长期目标

- [ ] 微服务拆分（Dubbo/gRPC）
- [ ] 容器化部署（Docker/K8s）
- [ ] 链路追踪（SkyWalking）
- [ ] CI/CD 自动化

---

## 贡献指南

欢迎提交 Issue 和 Pull Request！

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

---

## 许可证

本项目仅供学习和考核使用。

---

## 联系方式

- **项目团队**: TopView2026 开发组
- **邮箱**: support@topview2026.com

---

**最后更新时间**: 2026-04-28





